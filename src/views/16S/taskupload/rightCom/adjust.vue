    ...
    <div v-if="FormList.length">
      <el-form ref="FormRef" :model="Formdata" :rules="Formdatarules">
        <dynamicForm ref="dynamicFormRef" :taskId="taskId" :FormList="FormList"
          :attachValue="explainData?.analysisComponents" :indexNum="indexNum" :Formdata="Formdata"
          :FormdatauploadList="FormdatauploadList" type="图表" :i18n="i18n"></dynamicForm>
      </el-form>
      <div class="pad-l-5 pad-t-20"><el-button @click="start" :disabled="props.fromBut"
          type="primary">{{ $t('common.yypz') }}</el-button>
      </div>
    </div>
    <div v-else>
...
watch(() => props.explainData, () => {
  FormList.value = props.explainData.chartComponents
  FormList.value.forEach(item => {
    item.launch = false
  })
  recursionForm(FormList.value)
  console.log(FormList.value, 'hhhhhhhhhhhhhhhh');
